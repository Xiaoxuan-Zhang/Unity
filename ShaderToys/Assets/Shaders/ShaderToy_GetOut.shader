Shader "Unlit/ShaderToy_GetOut"
{
    ...
    Hit sceneSDF(vec3 p) {
        ...
        Hit is1;
        is1.dist = sphereSDF(p1, 1.0);
        is1.matIndex = 1;

        p1 = doTranslate(p, vec3(1.0, 1.5, -2.0));
        Hit is2;
        is2.dist = sphereSDF(p1, 1.5);
        is2.matIndex = 2;
        rst = unionSDF(is1, is2);

        Hit is3;
        is3.matIndex = 3;

        is3.dist = sdPlane(p, vec4(0.0, 1.0, 0.0, 0.0));
        rst = unionSDF(rst, is3);

        is3.dist = sdPlane(p, vec4(0.0, 0.0, 1.0, 20.0));
        rst = unionSDF(rst, is3);

        is3.dist = sdPlane(p, vec4(1.0, 0.0, 0.0, 20.0));
        rst = unionSDF(rst, is3);
        ...
        is3.dist = sdPlane(p, vec4(0.0, -1.0, 0.0, 30.0));
        rst = unionSDF(rst, is3);
    ...
    vec3 shading(vec3 ro, vec3 p, vec3 normal, Light lightInfo, inout Material mat) 
    {
        ...
        vec3 Fl = albedo/MATH_PI; //lambert
    ...
    Material newMaterial(vec3 albedo, float metallic, float roughness, vec3 reflection, vec3 refraction) {
        Material mat;
        mat.albedo = albedo;
    ...
    Material getMaterial(int index) {
        Material mat[4];
        mat[0] = newMaterial(vec3(1.0, 1.0, 1.0), 0.5, 0.1, vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0)); //white
        mat[1] = newMaterial(vec3(0.5, 0.8, 0.5), 0.9, 0.2, vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0)); //green
        mat[2] = newMaterial(vec3(0.9, 0.9, 0.2), 0.9, 0.2, vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0)); //yellow
        mat[3] = newMaterial(vec3(0.8, 0.5, 0.5), 0.5, 0.3, vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0)); //pink
        if (index == 0) {
            return mat[0];
        } else if (index == 1) {
            return mat[1];
    ...
    vec4 main(vec2 fragCoord) 
    {
        ...
        vec2 mouse = vec2(0.01, 0.01) + iMouse.xy  / iResolution.xy ;
        mouse -= 0.5;
        
        vec3 ro = vec3(10.0 * cos(mouse.x * MATH_2PI), 6.0 + 5.0 * mouse.y, 10.0 * sin(mouse.x * MATH_2PI));
        vec3 ta = vec3(0.0, 4.0, 0.0);
        mat3 cam = getCamera(ro, ta);
        vec3 rd = normalize(mul(cam, vec3(uv, 1.0)));
        Light lightInfo;
        lightInfo.position = vec3(5.0 * sin(iTime), 10.0, 2.0);
        lightInfo.color = vec3(200.0, 200.0, 200.0);
        
        Hit icp;
        vec3 nor = vec3(0.0, 0.0, 0.0);  
        vec3 ori = ro;
        vec3 dir = rd;
        vec3 interP = vec3(0.0, 0.0, 0.0);
        ...
        col = vec3(1.0, 1.0, 1.0) - exp(-col * 0.5);//exposure
